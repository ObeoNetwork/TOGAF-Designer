...
public class ApplicationArchitecture {
	...
	public List<Location> getExternalLocationContainingPhysicalApplicationComponentOrActorTaggedInTheContainer(
			Container container) {
		...
		for (Label label : container.getLabels()) {
			for (Element element : label.getOwnedElements()) {
				if (element instanceof Actor) {
					...
					PhysicalApplicationComponent pac = (PhysicalApplicationComponent) element;
	...
	public List<Location> getExternalLocationContainingPhysicalApplicationComponentOrActorCommunicatingWithTaggedElement(
			Container container) {
		List<Location> result = new ArrayList<Location>();
		org.obeonetwork.dsl.togaf.contentfwk.contentfwk.ApplicationArchitecture applicationArchitecture = getApplicationArchitecture(container);
		BusinessArchitecture businessArchitecture = getBusinessArchitecture(container);
		for (Label label : container.getLabels()) {
	...
	private org.obeonetwork.dsl.togaf.contentfwk.contentfwk.ApplicationArchitecture getApplicationArchitecture(
			Container container) {
		org.obeonetwork.dsl.togaf.contentfwk.contentfwk.ApplicationArchitecture result = null;
		...
		while (result == null && iterator != null) {
			if (iterator instanceof EnterpriseArchitecture) {
				...
				for (Architecture architecture : ea.getArchitectures())
					if (architecture instanceof org.obeonetwork.dsl.togaf.contentfwk.contentfwk.ApplicationArchitecture) {
						result = (org.obeonetwork.dsl.togaf.contentfwk.contentfwk.ApplicationArchitecture) architecture;
					}
			} else {
