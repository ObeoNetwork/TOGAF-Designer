 ...
 * 
 */
public class DataArchitectureLogicalDataPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {

	
	public static String LOGICALDATA_PART = "Logical Data"; //$NON-NLS-1$
	 ...
	 * 
	 */
	public DataArchitectureLogicalDataPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject dataArchitecture, String editing_mode) {
		super(editingContext, dataArchitecture, editing_mode);
		parts = new String[] { LOGICALDATA_PART };
		repositoryKey = ContentfwkViewsRepository.class;
		partKey = ContentfwkViewsRepository.LogicalData.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final LogicalDataPropertiesEditionPart logicalDataPart = (LogicalDataPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(ContentfwkViewsRepository.LogicalData.Properties.logicalData_)) {
				datalogicalcomponentSettings = new ReferencesTableSettings(dataArchitecture, ContentfwkPackage.eINSTANCE.getDataArchitecture_LogicalComponents());
				logicalDataPart.initLogicalData(datalogicalcomponentSettings);
			}
			// init filters
			logicalDataPart.addFilterToLogicalData(new ViewerFilter() {
			...
			// Start of user code for additional businessfilters for data-logical-component
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			LogicalDataPropertiesEditionPart logicalDataPart = (LogicalDataPropertiesEditionPart)editingPart;
			if (ContentfwkPackage.eINSTANCE.getDataArchitecture_LogicalComponents().equals(msg.getFeature()) && isAccessible(ContentfwkViewsRepository.LogicalData.Properties.logicalData_))
				logicalDataPart.updateLogicalData();
