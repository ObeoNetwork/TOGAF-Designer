...
public class ContainerPropertiesEditionPartImpl extends CompositePropertiesEditionPart implements ISWTPropertiesEditionPart, ContainerPropertiesEditionPart {
		...
		composer = new PartComposer(containerStep) {
			...
			public Composite addToPart(Composite parent, Object key) {
				...
				if (key == ContentfwkViewsRepository.Container.RelatedElements.ownsElements) {
					return createOwnsElementsAdvancedReferencesTable(parent);
				}
				return parent;
			}
		};
	...
	protected Composite createAttributesGroup(Composite parent) {
		...
		attributesGroup.setText(ContentfwkMessages.ContainerPropertiesEditionPart_AttributesGroupLabel);
	...
	protected Composite createNameText(Composite parent) {
		SWTUtils.createPartLabel(parent, ContentfwkMessages.ContainerPropertiesEditionPart_NameLabel, propertiesEditionComponent.isRequired(ContentfwkViewsRepository.Container.Attributes.name, ContentfwkViewsRepository.SWT_KIND));
		...
		name.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				if (propertiesEditionComponent != null)
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ContainerPropertiesEditionPartImpl.this, ContentfwkViewsRepository.Container.Attributes.name, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.SET, null, name.getText()));
			}
		...
		SWTUtils.createHelpButton(parent, propertiesEditionComponent.getHelpContent(ContentfwkViewsRepository.Container.Attributes.name, ContentfwkViewsRepository.SWT_KIND), null); //$NON-NLS-1$
		return parent;
	}
	...
	protected Composite createRelatedElementsGroup(Composite parent) {
		...
		relatedElementsGroup.setText(ContentfwkMessages.ContainerPropertiesEditionPart_RelatedElementsGroupLabel);
	 ...
	 * 
	 */
	protected Composite createOwnsElementsAdvancedReferencesTable(Composite parent) {
		...
		this.ownsElements.setHelpText(propertiesEditionComponent.getHelpContent(ContentfwkViewsRepository.Container.RelatedElements.ownsElements, ContentfwkViewsRepository.SWT_KIND));
		this.ownsElements.createControls(parent);
		this.ownsElements.addSelectionListener(new SelectionAdapter() {
			
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ContainerPropertiesEditionPartImpl.this, ContentfwkViewsRepository.Container.RelatedElements.ownsElements, PropertiesEditionEvent.CHANGE, PropertiesEditionEvent.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
		...
		"owns Elements", propertiesEditionComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			...
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					...
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ContainerPropertiesEditionPartImpl.this, ContentfwkViewsRepository.Container.RelatedElements.ownsElements,
	...
	protected void moveOwnsElements(EObject element, int oldIndex, int newIndex) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ContainerPropertiesEditionPartImpl.this, ContentfwkViewsRepository.Container.RelatedElements.ownsElements, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.MOVE, element, newIndex));
		ownsElements.refresh();
	}
	...
	protected void removeFromOwnsElements(EObject element) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ContainerPropertiesEditionPartImpl.this, ContentfwkViewsRepository.Container.RelatedElements.ownsElements, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.REMOVE, null, element));
		ownsElements.refresh();
	}
