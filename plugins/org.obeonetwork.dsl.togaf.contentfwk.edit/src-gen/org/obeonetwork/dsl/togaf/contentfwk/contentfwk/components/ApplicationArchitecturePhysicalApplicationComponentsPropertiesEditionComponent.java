 ...
 * 
 */
public class ApplicationArchitecturePhysicalApplicationComponentsPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {

	
	public static String PHYSICALAPPLICATIONCOMPONENTS_PART = "Physical Application Components"; //$NON-NLS-1$
	 ...
	 * 
	 */
	public ApplicationArchitecturePhysicalApplicationComponentsPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject applicationArchitecture, String editing_mode) {
		super(editingContext, applicationArchitecture, editing_mode);
		parts = new String[] { PHYSICALAPPLICATIONCOMPONENTS_PART };
		repositoryKey = ContentfwkViewsRepository.class;
		partKey = ContentfwkViewsRepository.PhysicalApplicationComponents.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final PhysicalApplicationComponentsPropertiesEditionPart physicalApplicationComponentsPart = (PhysicalApplicationComponentsPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(ContentfwkViewsRepository.PhysicalApplicationComponents.Properties.physicalApplicationComponents_)) {
				...
				physicalApplicationComponentsPart.initPhysicalApplicationComponents(physicalApplicationComponentSettings);
			}
			// init filters
			physicalApplicationComponentsPart.addFilterToPhysicalApplicationComponents(new ViewerFilter() {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			PhysicalApplicationComponentsPropertiesEditionPart physicalApplicationComponentsPart = (PhysicalApplicationComponentsPropertiesEditionPart)editingPart;
			if (ContentfwkPackage.eINSTANCE.getApplicationArchitecture_PhysicalApplicationComponents().equals(msg.getFeature()) && isAccessible(ContentfwkViewsRepository.PhysicalApplicationComponents.Properties.physicalApplicationComponents_))
				physicalApplicationComponentsPart.updatePhysicalApplicationComponents();
