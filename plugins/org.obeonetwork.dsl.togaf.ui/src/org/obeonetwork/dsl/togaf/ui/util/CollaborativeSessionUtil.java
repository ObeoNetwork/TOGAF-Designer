...
public class CollaborativeSessionUtil {
	...
	private static CollaborativeSession createCollaborativeSession() {
		...
		try {
			...
			openSession();
			CDOResource remoteDAnalysisResource = repositoryManager
					.getOrCreateTransaction(collaborativeSession)
					.getOrCreateResource(VIEWPOINT_MODEL_URI);
			DAnalysis dAnalysis = (DAnalysis) remoteDAnalysisResource
					.getContents().get(0);
			DAnalysisSelector dAnalysisSelector = new SimpleAnalysisSelector(
					dAnalysis);
	...
	private static void prepareRepresentationsResource()
			throws RepositoryConnectionException {
		CDOTransaction transaction = repositoryManager
				.getOrCreateTransaction(collaborativeSession);
		CDOResource remoteDAnalysisResource = transaction
				.getOrCreateResource(VIEWPOINT_MODEL_URI);
		...
		collaborativeSession
				.addRemoteRepresentationsSet(remoteDAnalysisResource.getPath());
